...
final class OffGameViewController: UIViewController, OffGamePresentable, OffGameViewControllable {
    ...
    private func buildPlayerLabels() {
        ...
        let player1Label = labelBuilder(PlayerType.player1.color, player1Name)
        self.player1Label = player1Label
        view.addSubview(player1Label)
        player1Label.snp.makeConstraints { (make: ConstraintMaker) in
        ...
        vsLabel.snp.makeConstraints { (make: ConstraintMaker) in
            make.top.equalTo(player1Label.snp.bottom).offset(10)
            make.left.right.equalTo(player1Label)
            make.height.equalTo(20)
        }
        
        let player2Label = labelBuilder(PlayerType.player2.color, player2Name)
        self.player2Label = player2Label
        view.addSubview(player2Label)
        player2Label.snp.makeConstraints { (make: ConstraintMaker) in
            make.top.equalTo(vsLabel.snp.bottom).offset(10)
            make.left.right.height.equalTo(player1Label)
        }
