...
final class TicTacToeInteractor: PresentableInteractor<TicTacToePresentable>, TicTacToeInteractable, TicTacToePresentableListener {
    ...
    func placeCurrentPlayerMark(atRow row: Int, col: Int) {
        ...
        if let winner = checkWinner() {
            ...
            let boardIsFilled = board.flatMap { $0.compactMap { $0 } }.count >= GameConstants.fullBoardCount
            if boardIsFilled {
                presenter.announce(winner: nil) { [weak self] in
                    self?.listener?.gameDidEndDraw()
                }
            }
    ...
    func closeGame() {
        listener?.gameDidEnd(withWinner: self.checkWinner())
    }
